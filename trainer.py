...
class Trainer(object):
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            ...
            epoch_loss/num_batches, epoch_bleu/num_batches, np.exp(epoch_loss/num_batches), epoch_sentlen/num_batches, epoch_wp/num_batches, time.time() - start_time)
            print(message)
    ...
    def train(self):
        ...
        for epoch in range(1, self.num_epoch):
            ...
            for i in tqdm(range(num_batches)):
                ...
                output = self.model(src, tgt, tgtlng)
                ...
                if i%self.args.log_interval == 0 and i>0:
                    ...
                    epoch, i, self.train_loss/self.args.log_interval, self.train_bleu/self.args.log_interval, np.exp(self.train_loss/self.args.log_interval), self.train_sentlen/self.args.log_interval, self.train_wp/self.args.log_interval,time.time() - start_time)
                    print(message)
                    self.train_loss = 0
                    self.train_bleu = 0
                    self.train_sentlen = 0
